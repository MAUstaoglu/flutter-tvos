...
4. `NativeTvosBundle.build()` orchestrates the native side:
   ...
   - **Generate Swift packages** — `TvosSwiftPackageManager` writes `FlutterFramework` (empty: it only keeps each plugin's `../FlutterFramework` dependency resolvable) and the `FlutterGeneratedPluginSwiftPackage` umbrella (depends on `FlutterFramework` + every plugin shipping a `tvos/Package.swift`) into `tvos/Flutter/ephemeral/Packages/`. The Runner project references the umbrella, so SPM provides the plugins; the engine is staged into `BUILT_PRODUCTS_DIR` and embedded by the Runner target instead (see "Embedding and signing the Flutter engine" in CLAUDE.md). Always generated, even with no SPM plugins.
...
- **Private fields in base classes** — `CachedArtifacts`, `FlutterBuildSystem`, and related Flutter 3.44.1 classes use private fields (`_fileSystem`, `_platform`). Subclasses must pass all named parameters directly to the `super()` constructor.
